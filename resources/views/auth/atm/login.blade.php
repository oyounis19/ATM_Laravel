...
@section('content')
    <div class="screens">
        <div class="credit screen">
            <h2 class="text-white fw-bolder mb-3">{{ __('Login by Credit Card') }}</h2>
            <form action="{{ route('atm.login.submit') }}" method="POST">
                @csrf

                <div class="form-floating mb-3">
                    <input type="text" class="card-input-field form-control @error('card_id') is-invalid @enderror" name="card_id" id="credit-card"
                        placeholder="1234 5648 6542 3156" minlength="16" maxlength="19">
                    <label for="credit-card">{{ __('Enter your credit card number') }}</label>
                ...
                <div class="form-floating">
                    <input id="floatingPassword" type="password" name="password" class="form-control @error('password') is-invalid @enderror" maxlength="4" minlength="4"
                    placeholder="Password">
                    <label for="floatingPassword">{{ __('Enter your PIN code') }}</label>
                ...
                <button class="btn btn-primary mt-3 w-100" type="submit">{{ __('Login') }}</button>
            </form>

        </div>
        <div class="fingerPrint screen d-flex justify-content-center flex-column align-items-center">
            <h2 class="text-white fw-bolder">{{ __('Login by FingerPrint Image') }}</h2>
            ...
            <form action="{{ route('atm.login.submit') }}" method="post" enctype="multipart/form-data">
                @csrf

                <input type="file" class="btn btn-primary mb-3" name="Fingerprint_Image">
...
@section('after_content')
    <div class="popup serviceLogin flex-column" pop="true">
        ...
        <h2 class="text-white fs-1 mb-5">{{ __('Login') }}</h2>
        <form method="POST" class="w-100">
            ...
            <div class="form-floating mb-3">
                ...
                <label for="inputId">{{ __('ATM ID') }}</label>
            </div>
            <br>
            <button name="bLogin" class="btn btn-primary mt-3 w-100">{{ __('Login') }}</button>
        </form>
    </div>
@endsection

@section('js')
    <script src="{{asset('js/atm/script.js')}}"></script>
@endsection
